...
export async function cleanupGlobalRegistryEntry(projectId: string): Promise<void> {
...
export async function runProjectCreate(flags: Record<string, string>): Promise<CLIRunnerResult> {
...
export async function runProjectList(): Promise<CLIRunnerResult> {
  // Use node to run the CLI directly from the root project
  const rootDir = join(process.cwd(), '..')
  const cliPath = join(rootDir, 'shared', 'dist', 'tools', 'project-cli.js')
