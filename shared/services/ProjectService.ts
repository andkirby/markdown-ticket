...
import { IProjectService, IProjectDiscoveryService, IProjectConfigService, IProjectCacheService, IProjectFileSystemService } from './project/types.js';
...
export class ProjectService implements IProjectService {
  ...
  // Config
  getGlobalConfig() { return this.config.getGlobalConfig(); }
  getProjectConfig(p: string) { return this.config.getProjectConfig(p); }
  createOrUpdateLocalConfig(projectId: string, projectPath: string, name: string, code: string, description?: string, repository?: string, globalOnly?: boolean, ticketsPath?: string) { return this.config.createOrUpdateLocalConfig(projectId, projectPath, name, code, description, repository, globalOnly, ticketsPath); }
  updateProject(id: string, u: any) { this.config.updateProject(id, u); this.clearCache(); }
  configureDocuments(id: string, p: string[]) { return this.config.configureDocuments(id, p); }

  // Discovery
  getRegisteredProjects() { return this.discovery.getRegisteredProjects(); }
  autoDiscoverProjects(p: string[]) { return this.discovery.autoDiscoverProjects(p); }
  registerProject(p: Project) { return this.discovery.registerProject(p); }
  ...
  async getAllProjects(): Promise<Project[]> {
    ...
    if (globalConfig.discovery?.autoDiscover) {
      ...
      allProjects = allProjects.filter(p => { if (seenIds.has(p.id)) return false; seenIds.add(p.id); return true; });
    }
  ...
  isCacheValid() { return this.cache.isCacheValid(); }
  getCacheAge() { return this.cache.getCacheAge(); }
  setCacheTTL(t: number) { this.cache.setCacheTTL(t); }
  getAllProjectsFromCache() { return this.cache.getAllProjectsFromCache(); }
  setCachedProjects(p: Project[]) { this.cache.setCachedProjects(p); }
  ...
  // FS
  async getSystemDirectories(p?: string) { return this.fs.getSystemDirectories(p); }
  async checkDirectoryExists(p: string) { return this.fs.checkDirectoryExists(p); }
}
...
class ProjectFileSystemService implements IProjectFileSystemService {
  ...
  async getSystemDirectories(p?: string) {
    const fs = await import('fs/promises'), path = await import('path'), os = await import('os'), targetPath = path.resolve(p || os.homedir());
    try { if (!(await fs.stat(targetPath)).isDirectory()) throw new Error('Not a directory'); } catch { throw new Error('Directory not found'); }
    const entries = await fs.readdir(targetPath), dirs: Array<{name: string; path: string; isDirectory: boolean}> = [];
    for (const e of entries) { if (e.startsWith('.')) continue; try { const ep = path.join(targetPath, e); if ((await fs.stat(ep)).isDirectory()) dirs.push({ name: e, path: ep, isDirectory: true }); } catch {} }
    dirs.sort((a, b) => a.name.localeCompare(b.name)); return { currentPath: targetPath, parentPath: path.dirname(targetPath) === targetPath ? '' : path.dirname(targetPath), directories: dirs };
  }
  async checkDirectoryExists(p: string): Promise<{ exists: boolean }> {
    if (!p || typeof p !== 'string') return { exists: false };
    try {
      const fs = await import('fs/promises'), path = await import('path'), os = await import('os'), rp = path.resolve(p), allowed = [os.homedir(), '/tmp', '/var/tmp', process.cwd(), '/Users'];
      if (!allowed.some(ap => rp.startsWith(ap) || rp === ap)) { logQuiet(this.quiet, `⚠️ Path denied: ${rp}`); return { exists: false }; }
