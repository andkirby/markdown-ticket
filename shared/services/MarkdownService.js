...
export class MarkdownService {
  ...
  static parseMarkdownFile(filePath) {
    try {
      ...
      if (ticket) {
        ...
        ticket.lastModified = stats.mtime.toISOString();
        if (!ticket.dateCreated) {
          ticket.dateCreated = stats.birthtime.toISOString();
        }
      }
  ...
  static parseMarkdownContent(content, filePath = '') {
    try {
      ...
      return {
        ...
        filePath: filePath,
        // Ensure arrays for relationship fields
        relatedTickets: this.normalizeArray(metadata.relatedTickets),
        dependsOn: this.normalizeArray(metadata.dependsOn),
        blocks: this.normalizeArray(metadata.blocks)
      };
    } catch (error) {
  ...
  static generateYamlFrontmatter(ticket) {
    ...
    // Dates
    if (ticket.dateCreated) {
      const date = ticket.dateCreated instanceof Date ? ticket.dateCreated : new Date(ticket.dateCreated);
      lines.push(`dateCreated: ${date.toISOString()}`);
    }
    if (ticket.lastModified) {
      const date = ticket.lastModified instanceof Date ? ticket.lastModified : new Date(ticket.lastModified);
      lines.push(`lastModified: ${date.toISOString()}`);
    }
    if (ticket.implementationDate) {
      const date = ticket.implementationDate instanceof Date ? ticket.implementationDate : new Date(ticket.implementationDate);
      lines.push(`implementationDate: ${date.toISOString()}`);
    }
