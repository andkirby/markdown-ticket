...
import type { TestEnvironment as ITestEnvironment } from '../types.js'
...
export class TestEnvironment {
  ...
  async setup(): Promise<void> {
    ...
    try {
      ...
      if (!existsSync(this._tempDir))
        throw new TestFrameworkError(`Failed to create temp directory: ${this._tempDir}`, 'TEMP_DIR_CREATE_FAILED')
      mkdirSync(this._configDir, { recursive: true })
      if (!existsSync(this._configDir))
        throw new TestFrameworkError(`Failed to create config directory`, 'CONFIG_DIR_CREATE_FAILED')
  ...
  async cleanup(): Promise<void> {
    ...
    await Promise.allSettled(this._cleanupHandlers.map(async (handler) => {
      try {
        await handler()
      }
      catch (error) {
  ...
  private _ensureInitialized(): void {
    if (!this._tempDir || !this._isInitialized)
      throw new TestFrameworkError(ERR_NOT_INIT, 'ENV_NOT_INITIALIZED')
  }
  ...
  private _removeExitHandlers(): void {
    if (!this._exitHandler)
      return
    for (const signal of EXIT_SIGNALS) process.removeListener(signal, this._exitHandler)
