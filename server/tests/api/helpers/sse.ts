...
export class MockEventSource extends EventTarget {
  ...
  connect(): void { this.connected = true; this.dispatchEvent(new Event('open')); }
  disconnect(): void { this.connected = false; this.dispatchEvent(new Event('close')); }
  isConnected(): boolean { return this.connected; }
  receiveMessage(msg: SSEMessage): void { this.dispatchEvent(new CustomEvent(msg.event || 'message', { detail: msg })); }
}
...
export function parseSSEMessage(raw: string): SSEMessage {
  ...
  for (const line of raw.split('\n')) {
    if (line.startsWith('data:')) msg.data = line.slice(5).trim();
    else if (line.startsWith('event:')) msg.event = line.slice(6).trim();
    else if (line.startsWith('id:')) msg.id = line.slice(3).trim();
    else if (line.startsWith('retry:')) msg.retry = parseInt(line.slice(6), 10);
  }
  return msg;
}
...
export function parseSSEChunk(chunk: string): SSEMessage[] {
  const msgs: SSEMessage[] = []; const cur: string[] = [];
  for (const line of chunk.split('\n')) {
    if (line === '') { if (cur.length) msgs.push(parseSSEMessage(cur.join('\n'))); cur.length = 0; }
    else cur.push(line);
  }
  if (cur.length) msgs.push(parseSSEMessage(cur.join('\n')));
  return msgs;
}
...
export function assertEventSequence(actual: SSEMessage[], expected: ExpectedSSEEvent[]): void {
  ...
  expected.forEach((exp, i) => assertSSEEvent(actual[i], exp));
}
