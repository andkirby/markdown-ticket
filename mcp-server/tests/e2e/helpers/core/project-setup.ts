...
import { ProjectConfig, ProjectFactoryError, DefaultProjectConfig } from '../types/project-factory-types';
...
export class ProjectSetup {
  ...
  constructor(options: ProjectSetupOptions) {
    if (!options.testEnv) throw new ProjectFactoryError('TestEnvironment is required');
  ...
  async createProjectStructure(
    ...
    try {
      ...
      const configPackage = this.configGenerator.generateProjectConfigPackage(
        projectCode, projectName, finalConfig
      );
  ...
  private validateProjectCode(projectCode: string): void {
    ...
    if (!result.valid) throw new ProjectFactoryError(result.errors.map(e => e.message).join('; '));
  }
