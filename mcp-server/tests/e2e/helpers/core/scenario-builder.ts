...
export class ScenarioBuilder {
  ...
  constructor(options: ScenarioBuilderOptions) {
    if (!options.projectSetup)
      throw new ProjectFactoryError('ProjectSetup is required')
    if (!options.testDataFactory)
      throw new ProjectFactoryError('TestDataFactory is required')
  ...
  private async setupComplexDependencies(_projectCode: string, _crs: MCPResponse[]): Promise<void> {
