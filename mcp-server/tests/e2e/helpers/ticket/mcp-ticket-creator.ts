...
import type { TicketCreator, TicketData, TicketResult, ValidationError, ValidationResult, ValidationWarning } from '../types/project-factory-types'
...
export class McpTicketCreator implements TicketCreator {
  ...
  validate(data: TicketData): ValidationResult {
    ...
    if (!data.content?.trim())
      errors.push({ code: 'MISSING_CONTENT', message: 'Content is required', field: 'content' })
    if (data.title?.length > 100)
      warnings.push({ code: 'LONG_TITLE', message: 'Title longer than 100 characters', field: 'title' })
