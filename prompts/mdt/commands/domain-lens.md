...
**Conciseness rule**: Keep only constraints that materially affect architecture. If the section grows beyond the key invariants and boundaries, you're over-specifying.
...
- **Minimal output** — only the constraints that shape architecture
...
- **No module boundary prescriptions** — that's architecture's job
...
- [ ] Output is concise and architecture-shaping
