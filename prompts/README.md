# Markdown Ticket - Claude Code Commands

Simple commands for integrating with Markdown Ticket through Claude Code.

## Installation

Copy commands to your Claude Code commands folder:

```bash
cp prompts/mdt-ticket-creation.md ~/.claude/commands/
```

## Usage

1. Provide context for ticket creation in Claude Code
2. Run: `/mdt-ticket-creation`

## Contributing

To add new prompts, please submit a request on GitHub.