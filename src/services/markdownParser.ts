...
export function parseMarkdownTicket(content: string): Partial<Ticket> | null {
  try {
    ...
    return {
      ...
      relatedTickets: frontmatterData.relatedTickets ? frontmatterData.relatedTickets.split(',').map((s: string) => s.trim()).filter(Boolean).filter((s: string) => !/^(none|n\/a|null|undefined)$/i.test(s)) : [],
      dependsOn: frontmatterData.dependsOn ? frontmatterData.dependsOn.split(',').map((s: string) => s.trim()).filter(Boolean).filter((s: string) => !/^(none|n\/a|null|undefined)$/i.test(s)) : [],
      blocks: frontmatterData.blocks ? frontmatterData.blocks.split(',').map((s: string) => s.trim()).filter(Boolean).filter((s: string) => !/^(none|n\/a|null|undefined)$/i.test(s)) : [],
