...
export function ProjectAccents({
  ...
  const [accentEnabled, setAccentEnabled] = useState<boolean>(() => prefsMemo.accentEnabled ?? true)
  const [accentStyle, setAccentStyle] = useState<string>(() => prefsMemo.accentStyle ?? 'gradient')
  const [autocolor, setAutocolor] = useState<boolean>(() => prefsMemo.autocolor ?? true)
  ...
  const handleReset = useCallback(() => {
    if (!autocolor) {
      // Autocolor off: clear stored accent, fill fallback hex into input
      ...
      // Autocolor on: clear stored accent (fallback applies automatically)
      setCustomHex('')
      setValidationError(undefined)
      onAccentReset(selectedCode)
    }
