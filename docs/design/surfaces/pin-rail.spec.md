...
- Drag-to-pin: the rail (and, when empty, its drop affordance) is a drop target for board cards
  using the existing drag type `'ticket'` (`board-layout.spec.md`). No new DnD system.
...
├── PinRail (hidden when pin set empty AND not dragging a ticket)
│   ├── PinRailHeader
│   │   ├── CollapsedLabel "Pinned"            (vertical / collapsed strip treatment)
│   │   └── Divider                            (1px --border rule)
│   ├── PinList (overflow-y-auto, flex-col, gap-1.5)
│   │   └── PinItem[]                          (one per pin, recency-pinned-first)
│   │       ├── PinCode "042"                  (numeric part only, mono)
...
│   └── DropAffordance                         (dashed "+" button; always last; drop target when empty)
└── content (existing: <main> / ProjectView)
```

`PinRail` itself is the drop target when non-empty (the whole rail accepts drops); `DropAffordance`
is the visible drop target when the rail is empty. Both resolve to the same `useDrop({ type:
'ticket' })` and the same `onPin(ticket)` handler.

### Pin ordering

Recency-pinned-first. The most recently pinned ticket renders at the **top** of the list, above the
divider. Order is derived from `favoritedAt` (descending) on the client; the server stores the full
ordered list. Manual reordering is deferred (IDEA-002).

## Children

| Child | Component | Spec | Conditional |
|-------|-----------|------|-------------|
| PinRail | `src/components/PinRail/index.tsx` (new) | this file | ≥1 pin OR a board card is being dragged |
| PinRailHeader | inline in `PinRail` | — | rail visible |
| PinList | inline in `PinRail` | — | ≥1 pin |
...
| DropAffordance | inline in `PinRail` | — | always rendered as the trailing dashed "+" button; doubles as the empty-rail drop target |
...
├── CollapsedLabel "Pinned"   (text-[9px] uppercase tracking-wider text-subtle; collapsed-strip treatment)
├── Divider                   (w-5 h-px, background: var(--border); my-1)
├── PinList                   (flex-1, overflow-y-auto, w-full, flex-col, items-center, gap-1.5)
│   └── PinItem …             (recency-pinned-first)
└── DropAffordance            (w-8 h-8, rounded-md, border border-dashed border-app; trailing)
```
...
button.pin-item (relative, w-8 h-8, rounded-md, border border-app, bg-app,
                 flex items-center justify-center, text-[10px] mono font-semibold text-muted,
...
- **Drop target**: the whole `PinRail` (when non-empty) and the `DropAffordance` (always, and
  especially when the rail is otherwise empty). Both use `useDrop({ accept: 'ticket', drop: item =>
  onPin(item.ticket) })`.
...
- **Drop hover visual** (reuses the board's established pattern, `board-layout.spec.md`):
  `bg-blue-50/50 dark:bg-blue-950/30 ring-2 ring-blue-400/30` on the rail/affordance while a ticket
  is dragged over it. No new hover vocabulary.
- **Empty-rail reveal**: when `pins.length === 0` and a board-card drag is in progress, the rail
  reveals itself (width `48px`, the `DropAffordance` visible as a dashed "+") so the user has a
  target to drop on. Outside a drag, the empty rail renders nothing and takes `0px`.
...
- `DropAffordance` is a `<button>` / drop region with `aria-label="Drop a ticket here to pin it"`.
...
| code text | `.ticket-key` (mono / tabular-nums / 600) | numeric code, color-agnostic |
| label text | `--text-subtle` | "Pinned" collapsed-strip label |
| divider | `var(--border)` | 1px rule under the label |
