...
| TEST-reservations-expiry-index | C-16 | `cloud/test/maintenance.test.ts` — `EXPLAIN QUERY PLAN` for the reservation-expiry scan uses the `reservations_by_expiry` index and never full-scans `ticket_reservations` |
