...
- Port defaults scattered across multiple files: `server/server.ts`, `mcp-server/src/index.ts`, `shared/test-lib/config/ports.ts`, `frontend/vite.config.ts`
...
- [x] All Vite environment variables have TypeScript definitions _(2026-08-17: `VITE_BACKEND_URL`, `VITE_BACKEND_PORT`, `VITE_DISABLE_EVENTBUS_LOGS` added to `frontend/src/vite-env.d.ts`; `VITE_HMR_*` confirmed dead and removed instead of typed)_
...
- Port defaults centralized in `DEFAULT_PORTS` (`shared/utils/constants.ts`); mirrored inline as `VITE_DEFAULT_PORTS` in `frontend/vite.config.ts` to avoid a build-time dependency on the `@mdt/shared` artifact.
...
`feat(MDT-157): loopback-host no-auth carve-out + bind boundary` landed on `origin/main` while this work was stashed and edited the same hunks (`server.ts` PORT/HOST region, `frontend/vite.config.ts` host/port region, compose `environment:` blocks). Resolved by keeping **both**: the MDT-117 port rename **and** MDT-157's `API_BIND_ADDRESS`/`API_LOCAL_HOST_BYPASS`/`VITE_SERVER_HOST` additions. The two are orthogonal.
