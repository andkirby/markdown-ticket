...
### UAT Session 2026-09-12
- Bug found in live GPDE UAT: `[GPDE-012](../.tickets/GPDE-012-learning-coverage-matrix.md)` in `research/learning-coverage-matrix-brief.md` rendered the EXISTING ticket GPDE-012 as a crossed-out "Document not found" span. Root cause (verified live against the served document tree): documents-view mode in `resolveDocumentRef` routed the tickets-area target to the documents view, and SmartLink's existence check treated "not in the index" as missing — but the index never covers the tickets area (GPDE excludes `.tickets`; tree roots are `./`, `docs`, `gpt`, `research`). The round-2 watchlist's anticipated "extend D12 routing to documents-view mode" use case
- Approved: tickets-area targets mean the ticket, in every source mode — `<ticketsPath>/<KEY>[-slug].md` → ticket view, `<ticketsPath>/<KEY>/<rest>.md|.html` → that ticket's subdoc view; bare ticket-key-shaped .md basenames in documents-view mode route to the ticket mirroring ticket-context semantics; ticket-context mode no longer escapes another ticket's subdoc to a fabricated documents path (new BR-2.7, architecture D13, part of OBL-8)
- Approved: known-missing flagging is coverage-bounded — SmartLink flags broken only when the index positively covers the target's directory (`coversPrefix`, first consumer) and the file is absent; tickets-area/unconfigured prefixes are unknown, rendered as normal links; BR-2.1 covered-and-missing behavior unchanged (new BR-2.8, architecture D14, part of OBL-8)
- Approved (same session): component-test harness repair — happy-dom now registers with a real origin (`http://localhost:3075/`); under `about:blank` the existence check's `new URL(href, origin)` threw, silently forcing "not broken" for every component test of this path
- Found during live verification (mock/wiring gap): unit tests passed `.tickets` explicitly, but the documents-view MarkdownViewer never forwarded `ticketsPath` to MarkdownContent, so the routing fix was inert in the live app (preprocessor defaulted to docs/CRs); fixed by fetching the project config in MarkdownViewer and forwarding the configured ticketsPath (BR-2.7 wiring, TEST-unit-docsview-ticketspath); MarkdownViewer's fetch mock upgraded from queue-based fixtures to URL-aware routing
- Updated docs: requirements.md, architecture.md, tests.md, tasks.md, uat.md; all trace stages re-validated (strict) and re-rendered; requirements lock baseline written
- Verified: mdt237 suite 50/50 (8 new BR-2.7 cases), SmartLink component suite 5/5 (new), MarkdownViewer suite 9/9 (2 new wiring cases + URL-aware mock), cache suite +3 coversPrefix cases, full frontend suite 1079/0, eslint + validate:ts clean on touched files
- Live verified on GPDE (running app): GPDE-012 renders as a ticket link to /prj/GPDE/ticket/GPDE-012 with no broken flag; click navigates to the ticket view and opens the ticket