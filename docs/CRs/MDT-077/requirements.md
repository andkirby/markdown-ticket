...
1. WHEN creating a project with valid parameters, the system shall create the project with auto-generated uppercase code or user-provided code (accepts lowercase input, converts to uppercase).
...
| Concern | Requirement |
|---------|-------------|
| C1: Legacy Config | SHALL NOT support legacy configuration formats or fields not in specification |
| C2: ProjectConfig Removal | MUST delete ProjectConfig class and use LocalProjectConfig only |
| C3: Shared API Usage | Configuration generation must use shared ConfigurationGenerator.generateMdtConfig API |
| C4: Repository Pattern | Project configuration (.toml) writes must go through repository class only |
| C5: Code Format | Project codes must be 2-5 uppercase letters, auto-converted from user input |
| C6: Test Coverage | CLI operations must achieve >90% test coverage |
| C7: Concurrency | Concurrent operations must not corrupt project data |
| C8: Interface Consistency | All interfaces (CLI, Web UI, MCP) must use shared business logic |

## Constraint Carryover

| Constraint ID | Must Appear In |
|---------------|----------------|
| C1 | architecture.md (Configuration Schema section), tasks.md (Verify no legacy code) |
| C2 | architecture.md (ProjectConfig removal), tasks.md (Delete and refactor) |
| C3 | architecture.md (Shared Services section), tasks.md (Refactor generation code) |
| C4 | architecture.md (Repository Pattern section), tests.md (Repository tests) |
