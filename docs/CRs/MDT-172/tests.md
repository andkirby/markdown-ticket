...
| Frontend read-only controls | `frontend/src/components/ReadOnlyMode.test.tsx` | board/list/documents controls unavailable in read-only state |
...
| C4 | `server/tests/api/public-sharing.test.ts`, `frontend/src/components/ReadOnlyMode.test.tsx` | backend 403 gate plus UI control removal |
