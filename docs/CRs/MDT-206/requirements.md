...
| Toolbar search (swimlane) | A search field in the swimlane toolbar (beside Hide empty / Show badges / Show closed) filters the **epic lane list by the epic ticket's own key or title** — matching runs on **epic tickets only**. An epic lane stays when its epic key/title matches (showing all its tickets); every other lane, **including the No-epic lane**, is removed while the query is active. Matching is by **title only** (case-insensitive substring) or **ticket key**, accepting `ABC-012` (full zero-padded), `12` (bare number), and `ABC-12` (simplified, normalized to zero-padded). Presentation-only: no ticket mutation, epic progress unchanged. | Matching **child tickets** (title/key of tickets inside lanes) — the search narrows the epic list, not the cards inside it; that is the header global filter's job. Also: searching fields other than title/key. | A "narrow the board to the epics I care about" control — the lane list is the filter target, and the query means "which epic", never "which ticket". |
