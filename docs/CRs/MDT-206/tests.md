...
- Helper + component + E2E tests cover the swimlane **toolbar search** (BR-6.1): title substring (case-insensitive), full zero-padded key, bare number, and simplified key (`ABC-12` → `ABC-012`), and **lane-list filtering** (an epic key/title match keeps the whole lane, a child-only match narrows the lane, non-matching epic and No-epic lanes are removed entirely), presentation-only filtering (epic progress unchanged), and the clear button restoring the full board.
