...
| Project init behavior | `mdt-cli project init` bootstraps `.mdt-config.toml`, related project metadata, and the marker-delimited MDT working-state `.gitignore` block in the current folder (idempotent merge; unmanaged lines untouched) | Require a separate top-level init command, reuse ticket creation flow, or leave MDT working-state ignores to each repo's hand-maintained `.gitignore` | Aligns with the requested parity with `bun run project:create` and keeps generated MDT artifacts out of git status in new projects |
...
## UAT Refinements (2026-09-03)

- BR-16 refined: `project init` also ensures the target folder's `.gitignore` contains a marker-delimited managed MDT block, creating `.gitignore` when absent and merging into an existing one without altering unmanaged content. The canonical entry list, its evidence, and the git policy per file live in [docs/MDT_WORKING_STATE_FILES.md](../../MDT_WORKING_STATE_FILES.md) (single durable source; the shared scaffold constant must mirror it). The block ends with a `{ticketsPath}/{CODE}-*.md` negation so ticket files stay trackable when a slug collides with an ignore pattern, omits broad dot-file patterns that swallow tool configs, and leaves the spec-trace store under `{ticketsPath}/.trace/` trackable.
- Edge-12 added: init into a folder with an existing user-authored `.gitignore` preserves every unmanaged line, appends the managed block exactly once, and a second init run leaves the file unchanged (idempotent merge, no duplicate block)
- C10 added: the MDT ignore entry list is owned by one shared constant consumed by every entrypoint routing through `ProjectManager.createProject` (`mdt-cli project init`, `project:create` script flow, web-driven creation), so scaffolded ignores cannot drift between interfaces; the list is phrased canonically in docs/MDT_WORKING_STATE_FILES.md; the block is marker-delimited and re-mergeable
