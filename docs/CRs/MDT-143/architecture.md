...
| Structured output | **Build** `cli/src/output/structured.ts` | LLM agents need deterministic JSON/YAML objects instead of scraping human terminal output |
...
| `cli/src/output/structured.ts` | Agent output layer | Serialize the same command result object as JSON or YAML with stable schema fields, structured errors, no ANSI, and no decorative separators |
...
cli/
  ...
  src/
    ...
    output/
      formatter.ts
      colors.ts
      structured.ts              # --json/--yaml serialization and error envelopes
...
10. **Structured output is first-class**: JSON/YAML output is generated from command result objects, not parsed from human formatter strings.
11. **One structured schema**: YAML is a serialization of the same JSON-compatible object used for `--json`.
12. **Errors stay parseable for agents**: In structured mode, failures write an `ok=false` envelope to stderr and do not mix partial success payloads into stdout.
...
Colors: ticket key (MDT-012) light-cyan bold, title white bold, all file paths gray. Directories have trailing `/`. If no subdocuments exist, the directory section is omitted.
...
```json
{
  ...
  "data": {
    "ticket": {
      "key": "MDT-012",
      "title": "Add CLI access to tickets and projects",
      "status": { "value": "implemented", "label": "Implemented" },
      "paths": {
        "relative": "docs/CRs/MDT-012-add-cli-access.md",
        "absolute": "<project-root>/docs/CRs/MDT-012-add-cli-access.md"
      },
      "subdocuments": [
        { "path": "docs/CRs/MDT-012/bdd.md", "kind": "file" }
      ]
    }
  },
  "meta": {
...
YAML mode serializes the same object. Success payloads go to stdout. Structured failures use `ok: false` on stderr and keep stdout empty.
