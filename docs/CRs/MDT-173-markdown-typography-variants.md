...
### Problem
- `frontend/src/components/MarkdownContent/index.tsx` applies one default `prose prose-sm max-w-none` class to all primary markdown reading surfaces.
- `frontend/src/styles/prose.css` styles code, tables, anchors, Mermaid, and Wireloom but does not define durable typography rhythm for headings, paragraphs, lists, blockquotes, tables, or reading-width variants.
- `frontend/src/components/DocumentsView/MarkdownViewer.tsx` renders full documents without a document-specific prose measure, causing wide desktop lines and broken mobile reading with the persistent navigation pane.
- `frontend/src/components/TicketViewer/index.tsx` renders ticket markdown without an explicit compact ticket prose contract, so section rhythm and timestamp spacing are implicit.

### Affected Artifacts
- `frontend/src/components/MarkdownContent/index.tsx` - prose variant class contract and default behavior.
- `frontend/src/styles/prose.css` - markdown typography, rich content styling, and prose variants.
- `frontend/src/config/settingsPreferences.ts` - browser-local Markdown Density preference with Compact default.
- `frontend/src/components/SettingsModal.tsx` - Settings control for Markdown Density.
- `frontend/src/components/DocumentsView/MarkdownViewer.tsx` - document prose variant usage and viewer content measure.
- `frontend/src/components/DocumentsView/documents-view.css` - document viewer layout and responsive reading behavior.
- `frontend/src/components/TicketViewer/index.tsx` - ticket prose variant usage and timestamp/content spacing.
- `frontend/src/components/MarkdownContent/*.test.tsx` - renderer and class contract coverage.
- `frontend/src/components/DocumentsView/*.test.tsx` - document viewer variant and responsive behavior coverage.
- `frontend/src/components/TicketViewer/*.test.tsx` - ticket viewer variant and timestamp/content coverage.
...
### Rationale
- `frontend/src/STYLING.md` already assigns prose and markdown rendering to `frontend/src/styles/prose.css`, so variant classes fit the existing CSS architecture.
...
- Tokenized CSS keeps dark mode and code-theme behavior aligned with `frontend/src/styles/design-tokens.css`.
...
|----------|-------------|--------------|
| `frontend/src/styles/prose.css` | CSS expanded | Add `.prose--document`, `.prose--ticket`, `.prose--compact`, and tokenized descendant styles |
| `frontend/src/config/settingsPreferences.ts` | Preference added | Add `MarkdownDensity` type, `getMarkdownDensity()`, `setMarkdownDensityPreference()`, and storage key `markdown-ticket:settings:markdown-density` |
| `frontend/src/components/SettingsModal.tsx` | Control added | Add Markdown Density select with Compact, Default, Comfortable options |
| `frontend/src/components/MarkdownContent/index.tsx` | Class contract updated | Preserve default rendering while allowing stable variant classes from consumers |
| `frontend/src/components/DocumentsView/MarkdownViewer.tsx` | Variant applied | Render document markdown with `.prose--document` and active markdown density class |
| `frontend/src/components/DocumentsView/documents-view.css` | Layout updated | Add readable measure and responsive single-pane preview behavior |
| `frontend/src/components/TicketViewer/index.tsx` | Variant applied | Render ticket markdown with `.prose--ticket` and active markdown density class; avoid timestamp overlap |
| `frontend/src/components/MarkdownContent/useHtmlParser.ts` | Link focus compatibility checked | Preserve SmartLink replacement and heading-anchor behavior |
| `frontend/src/components/MarkdownContent/useMarkdownProcessor.ts` | Pipeline compatibility checked | Preserve markdown-it plugin behavior |
| `frontend/src/components/MarkdownContent/useMarkdownProcessor.test.ts` | Tests updated | Verify markdown output remains compatible with new prose classes |
| `frontend/src/config/settingsPreferences` tests | Tests updated | Verify Markdown Density defaults to Compact and rejects invalid stored values |
| `frontend/src/components/DocumentsView/MarkdownViewer.test.tsx` | Tests updated | Verify document prose variant is applied |
| `frontend/src/components/TicketViewer` tests | Tests updated | Verify ticket prose variant is applied |
...
### Key Patterns
- CSS modifier pattern: `.prose--document`, `.prose--ticket`, `.prose--compact` follow `frontend/src/STYLING.md` structural modifier guidance.
- Tokenized theme pattern: markdown colors use existing CSS variables from `frontend/src/styles/design-tokens.css`.
...
### Functional
- [ ] `frontend/src/styles/prose.css` defines `.prose--document`, `.prose--ticket`, and `.prose--compact` modifiers.
- [ ] `frontend/src/styles/design-tokens.css` defines `--code-inline-fg` as `217 70% 38%` in light mode and `213 94% 78%` in dark mode.
- [ ] `frontend/src/config/settingsPreferences.ts` defines Markdown Density values: `compact`, `default`, `comfortable`.
...
- [ ] `frontend/src/styles/prose.css` defines explicit markdown rhythm for headings, paragraphs, lists, task lists, blockquotes, horizontal rules, links, inline code, code blocks, tables, and images.
...
- [ ] `frontend/src/styles/prose.css` uses theme tokens for prose text, links, borders, code, selection, fallback Wireloom states, and focus treatment.
- [ ] Markdown links expose hover and keyboard focus treatment using `--primary` and `--ring`.
- [ ] `frontend/src/components/DocumentsView/MarkdownViewer.tsx` renders markdown with the document prose variant.
...
- [ ] `frontend/src/components/TicketViewer/index.tsx` renders markdown with the ticket prose variant.
...
### Non-Functional
- [ ] Styling follows `frontend/src/STYLING.md` class taxonomy and CSS location rules.
- [ ] No markdown typography rules are added to `frontend/src/styles/base.css` except existing global heading defaults.
...
### By CR Type
- Feature: `frontend/src/styles/prose.css` contains the three prose variants, density targets, reading-measure rules, and tokenized markdown descendant styles.
...
- Modify `frontend/src/config/settingsPreferences.ts`.
...
- Move markdown rhythm into `.prose` descendants, not `frontend/src/styles/base.css`.
...
- Modify `frontend/src/components/DocumentsView/MarkdownViewer.tsx`.
...
- Modify `frontend/src/components/TicketViewer/index.tsx`.
...
- Modify `frontend/src/components/DocumentsView/documents-view.css`.
...
### Phase 7: Unit Tests
- Update `frontend/src/config/settingsPreferences` tests for Markdown Density.
- Update SettingsModal tests for the Markdown Density control.
- Update `frontend/src/components/DocumentsView/MarkdownViewer.test.tsx` to assert document prose variant class usage.
...
- Do not add markdown typography rules to `frontend/src/styles/base.css`.
