...
MDT-181 adds per-user, per-project accent colors to the project selector and browser surfaces. Users choose one of 16 preset colors or a custom hex value from the Project Edit form. The accent provides stronger visual identity in the selector rail and browser cards without increasing component dimensions. A single stored hex value drives both light and dark mode rendering. The feature is a personal visual preference — never written to shared project configuration.
...
| Edit form section | Visually separated "Your Project Accent" section within AddProjectModal (edit mode), persisted via `/api/config/selector` independently of shared project update | Inline with shared project fields; separate modal | CR requires clear separation between personal preference and shared project metadata. Same form, separate section, separate persistence path. |
