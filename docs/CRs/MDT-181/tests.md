...
| `frontend/src/utils/accentColors.ts` | `frontend/src/utils/__tests__/accentColors.test.ts` | 16-color palette, WCAG AA foreground contrast, deterministic fallback, hex validation |
| `frontend/src/components/ProjectSelector/useSelectorData.ts` | `frontend/src/components/ProjectSelector/useSelectorData.test.ts` | accent load, accent persistence, cross-user/read-only no-owner-state isolation guard |
...
| `frontend/src/components/ProjectSelector/ProjectSelectorChip.tsx` + `ProjectSelectorCard.tsx` | `frontend/src/components/ProjectSelector/ProjectSelectorAccent.test.tsx` | chip accent mark, card identity fill, repeated-accent rendering, no-image fallback path |
...
| C3 | `frontend/src/components/ProjectSelector/ProjectSelectorAccent.test.tsx`, `tests/e2e/selector/project-accent-colors.spec.ts` | chip/card identity keeps compact structure in unit tests and equal rendered height in E2E |
| C4 | `frontend/src/utils/__tests__/accentColors.test.ts` | 16-color palette + contrast verification |
| C5 | `frontend/src/utils/__tests__/accentColors.test.ts` | luminance-based foreground auto-selection |
| C6 | `frontend/src/utils/__tests__/accentColors.test.ts` | deterministic fallback algorithm |
...
|---------|-----------|-------|
| Edge-1 | `frontend/src/utils/__tests__/accentColors.test.ts` | unset accent resolves to deterministic fallback |
...
| Edge-4 | `frontend/src/components/ProjectSelector/ProjectSelectorAccent.test.tsx` | repeated accent values render independently |
| Edge-5 | `frontend/src/components/ProjectSelector/ProjectSelectorAccent.test.tsx` | long labels keep identity marks present |
| Edge-6 | `tests/e2e/selector/project-accent-colors.spec.ts` | theme switch updates without re-selection |
| Edge-7 | `frontend/src/components/ProjectSelector/useSelectorData.test.ts`, `server/tests/api/selector.test.ts` | no-owner-state path does not load or persist owner selector changes |
| Edge-8 | `frontend/src/components/ProjectSelector/ProjectSelectorAccent.test.tsx` | accent remains sole identity source without image |
| Edge-9 | `frontend/src/components/ProjectSelector/ProjectSelectorAccent.test.tsx` | no image autodiscovery path assumed by card rendering |
