...
| `frontend/src/utils/routing.ts` | `frontend/src/utils/__tests__/routing.normalizeTicketKey.test.ts` | 9 | GREEN (regression) |
...
2. The `filterTickets()` matching logic is already tested in the existing `frontend/src/hooks/useQuickSearch.test.ts` (MDT-136)
...
grep -rn "padStart(3" --include="*.ts" shared/ frontend/src/ server/
```
...
8. `frontend/src/utils/routing.ts:15`
9. `frontend/src/hooks/useQuickSearch.ts:93`
