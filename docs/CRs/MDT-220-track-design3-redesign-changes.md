...
- Scope: the `designs/board-zai/` prototype today; the real-app port (`frontend/src/styles`, `frontend/src/components`) as it begins
...
| Tier-led depth (borders dropped), background tiers, mono ticket codes | Established | inherent Design3 grammar; frontend/src/ port drops card/column/badge borders, keeps only the data-driven accent stripe |
| Token bridge to app: tuned dark-theme ramp propagated to frontend/src/styles/design-tokens.css .dark (--bg-subtle #1a1f2b / --bg-muted #232b3a / --bg-elevated #2c3848, matching the prototype) | Done | frontend/src/styles/design-tokens.css |
...
- Dropped (Done): card static gray border -> transparent, revealed on hover only (.kanban-card); column outer border removed, defined by bg-subtle tier + radius + gap (.board-col); modal badge outlines (priority/type/phase/sprint) -> flat bg-muted tint — design3.html, now ported to frontend/src/ (ticket.css, column.css, badge.css)
...
| Item | Status | Note |
| --- | --- | --- |
| Propagate tuned dark-theme ramp into app `.dark` tokens | Done | frontend/src/styles/design-tokens.css `.dark` = #1a1f2b / #232b3a / #2c3848; browser-verified live |
