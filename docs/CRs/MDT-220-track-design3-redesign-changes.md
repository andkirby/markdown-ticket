...
- Border audit (Done): grep of all border usage across src/. Findings — the shadcn `Card` primitive (`ui/Card.tsx`) and the `.card` class (`layout.css`) are **dead code** (0 consumers), so the earlier "#1 drop target" was a dead-code discovery, not a real surface. Most *rendered* borders are load-bearing (form controls, ResolutionDialog selection affordance, structural separators, floating layers, AuthUnlock panel) and stay. The one genuinely decorative surface — ReadAccessTokens info panels — was converted border→`bg-muted` tier (vanish-proof by construction). Remaining, intentionally not touched: ResolutionDialog option cards (selection border = meaning, keep) and DuplicateResolver hardcoded inline `#dee2e6`/`#dc3545` (theming-contract hygiene — separate token-cleanup pass).
...
- Surface contract for controls (Done): codified in STYLING.md + demonstrated in styleguide.html — a control's fill must differ ≥3:1 from its container or be border-defined (else it ghosts). Measured: `--bg-muted` is only **1.23:1** from `--card`/`--bg-elevated` in light, so a borderless muted input — and `btn-secondary` (`--secondary` ≈ `--muted`) — ghost on white. `btn-secondary` is therefore **border-defined** (its border is affordance, not decoration). `--bg-muted` is untouchable (the app's universal hover tier, ~36 files), so borderless inputs need a dedicated recessed fill (Material-3 `#bcc7d4` / `#384560`, 1.71:1) applied to *all* input classes at once. Treatment choice (gray-fill borderless vs hairline border) pending; `.input` migration held to avoid a piecemeal half-borderless look.
