...
**Durable docs**: deferred to the implementation commit, mirroring MDT-244's rule (durable docs describe final behavior, not drafts — updating them now would spec unwired behavior). At implementation: `frontend/src/STYLING.md` § Stable Scanning Patterns gains the status-marker bullet (between Priority and Type); `docs/design/surfaces/pin-rail.spec.md` + `pin-rail.mockups.md` swap the tooltip's StatusBadge for the strip icon (spec lines 37, 105, 131, 236-244; mockup `tt-status` annotation at lines 70-74); `styleguide.html` may gain a status-icons section beside `#type-icons`. List view has no canonical surface spec today — the Status-column contract lives in this CR; decide at implementation whether a `list-view.spec.md` is warranted.
