...
const TicketAttributeTags: React.FC<TicketAttributeTagsProps> = ({
  ...
  useEffect(() => {
    const handleBadgePreferenceChange = (event: Event) => {
      const badgeIds = (
        event as CustomEvent<{ badgeIds?: TicketCardBadgeId[] }>
      ).detail?.badgeIds
  ...
  const isVisible = (badgeId: TicketCardBadgeId) =>
    visibleBadgeIds.includes(badgeId)
  ...
  return (
    <div className={`flex flex-wrap gap-2 ${className}`}>
      ...
      {isVisible(TicketCardBadge.RELATED)
        && ticket.relatedTickets
        && ticket.relatedTickets.length > 0 && (
        <RelationshipBadge
          variant="related"
          links={ticket.relatedTickets}
          displayMode="compact"
        />
      )}
      {isVisible(TicketCardBadge.DEPENDS)
        && ticket.dependsOn
      ...
      {isVisible(TicketCardBadge.BLOCKS)
        && ticket.blocks
